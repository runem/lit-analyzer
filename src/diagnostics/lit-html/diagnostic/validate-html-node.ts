...
export function validateHtmlNode(htmlDocument: HtmlDocument, htmlNode: HtmlNode, store: TsLitPluginStore): LitHtmlDiagnostic[] {
	...
	if (htmlTag == null) {
		...
		reports.push({
			...
			tip: `Please consider adding it to the 'globalTags' plugin configuration or disabling the check using 'skipUnknownTags'.`,
			location: htmlNode.location.name,
			severity: "warning",
			htmlNode,
			suggestedName
		...
		if (!store.config.skipMissingImports) {
			...
			if (isCustomElement && !isDefinitionImported && definition != null) {
				...
				reports.push({
					...
					tip: `You can disable this check using 'skipMissingImports'`,
					severity: "warning",
					location: htmlNode.location.name,
					htmlNode,
					definition,
