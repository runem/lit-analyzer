...
export interface VisitContext {
	checker: TypeChecker;
	parent?: TextDocument;
	emitTextDocument(document: TextDocument): void;
...
function visitTaggedTemplateExpression(astNode: TaggedTemplateExpression, context: VisitContext): TextDocument | undefined {
	...
	if (context.shouldCheckTemplateTag(templateTag)) {
		...
		const textDocument = new TextDocument(templateTag, astNode, { literalParts, expressionParts });
